...
/**
 * InvoicePayerInfo Class Doc Comment
 *
 * @category Class
 ...
 */
class InvoicePayerInfo implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'InvoicePayerInfo';
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['inn']) && !preg_match("/^(\\d{12}|\\d{10})$/", $this->container['inn'])) {
            $invalidProperties[] = "invalid value for 'inn', must be conform to the pattern /^(\\d{12}|\\d{10})$/.";
        }
     ...
     * Sets inn
     *
     * @param string|null $inn Инн плательщика
     *
     * @return self
     */
    public function setInn($inn)
    {
        ...
        if (!is_null($inn) && (!preg_match("/^(\\d{12}|\\d{10})$/", $inn))) {
            throw new \InvalidArgumentException("invalid value for $inn when calling InvoicePayerInfo., must conform to the pattern /^(\\d{12}|\\d{10})$/.");
        }
     ...
     * Sets kpp
     *
     * @param string|null $kpp Кпп плательщика
     *
     * @return self
     */
    public function setKpp($kpp)
    {
        ...
        if (!is_null($kpp) && (!preg_match("/^(\\d{9})$/", $kpp))) {
            throw new \InvalidArgumentException("invalid value for $kpp when calling InvoicePayerInfo., must conform to the pattern /^(\\d{9})$/.");
        }
     ...
     * Sets name
     *
     * @param string|null $name Наименование плательщика
     *
     * @return self
     */
    public function setName($name)
    {
        if (!is_null($name) && (mb_strlen($name) > 512)) {
            throw new \InvalidArgumentException('invalid length for $name when calling InvoicePayerInfo., must be smaller than or equal to 512.');
        }
        if (!is_null($name) && (mb_strlen($name) < 1)) {
            throw new \InvalidArgumentException('invalid length for $name when calling InvoicePayerInfo., must be bigger than or equal to 1.');
        }
