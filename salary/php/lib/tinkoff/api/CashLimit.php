 ...
 */
class CashLimit implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'CashLimit';
    ...
    protected static $setters = [
        'limit_period' => 'setLimitPeriod',
        'limit_remain' => 'setLimitRemain',
    ...
    protected static $getters = [
        'limit_period' => 'getLimitPeriod',
        'limit_remain' => 'getLimitRemain',
    ...
    const LIMIT_PERIOD_IRREGULAR = 'IRREGULAR';
    const LIMIT_PERIOD_CUSTOM = 'CUSTOM';
     ...
     * @return string[]
     */
    public function getLimitPeriodAllowableValues()
    {
        return [
            self::LIMIT_PERIOD_DAY,
            self::LIMIT_PERIOD_MONTH,
            self::LIMIT_PERIOD_IRREGULAR,
            self::LIMIT_PERIOD_CUSTOM,
        ];
    }
    ...
    {
        $this->container['limit_period'] = $data['limit_period'] ?? null;
        $this->container['limit_remain'] = $data['limit_remain'] ?? null;
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['limit_period'] === null) {
            $invalidProperties[] = "'limit_period' can't be null";
        }
        $allowedValues = $this->getLimitPeriodAllowableValues();
        if (!is_null($this->container['limit_period']) && !in_array($this->container['limit_period'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'limit_period', must be one of '%s'",
                $this->container['limit_period'],
                implode("', '", $allowedValues)
            );
        }

        if ($this->container['limit_remain'] === null) {
            $invalidProperties[] = "'limit_remain' can't be null";
        }
        if ($this->container['limit_value'] === null) {
            $invalidProperties[] = "'limit_value' can't be null";
        }
    ...
    {
        return $this->container['limit_period'];
    }

    /**
     * Sets limit_period
     *
     * @param string $limit_period Период обновления доступного остатка лимита на снятия наличных:<ul> <li>\"DAY\" &ndash; доступный остаток станет равным лимиту в начале следующих суток</li> <li>\"MONTH\" &ndash; доступный остаток станет равным лимиту в начале следующего месяца</li> <li>\"IRREGULAR\" &ndash; доступный остаток станет равным лимиту только при обновлении лимита</li> <li>\"CUSTOM\" &ndash; период обновления лимита вручную установлен в дополнительное состояние, узнать которое вы можете у вашего персонального менеджера</li> </ul>
     *
     * @return self
     */
    ...
    {
        $allowedValues = $this->getLimitPeriodAllowableValues();
        if (!in_array($limit_period, $allowedValues, true)) {
        ...
        $this->container['limit_period'] = $limit_period;
     ...
     * @return float
     */
    public function getLimitRemain()
    {
        return $this->container['limit_remain'];
    }

    /**
     * Sets limit_remain
     *
     * @param float $limit_remain Доступный остаток лимита на снятия наличных в валюте счёта, к которому привязана карта
     *
     * @return self
     */
    public function setLimitRemain($limit_remain)
    {
        $this->container['limit_remain'] = $limit_remain;
     ...
     * Sets limit_value
     *
     * @param float $limit_value Лимит на снятия наличных в валюте счёта, к которому привязана карта
     *
     * @return self
     */
